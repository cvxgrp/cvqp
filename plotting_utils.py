...
def plot_proj_benchmarks(results, save_figures=False):
    ...
    return create_benchmark_plot(
        ...
        filter_solver=lambda r, s: sorted([x for x in r if x.solver == s], key=lambda x: x.m),
...
def plot_cvqp_benchmarks(results, save_figures=False):
    ...
    return create_benchmark_plot(
        results=results,
        get_groups=lambda r: sorted(set(x.n_vars for x in r)),  # Get all unique n_vars values
        filter_group=lambda r, n: [x for x in r if x.n_vars == n],  # Filter by n_vars
        get_solvers=lambda r: sorted(set(x.solver for x in r)),
        filter_solver=lambda r, s: sorted([x for x in r if x.solver == s], key=lambda x: x.n_scenarios),
...
def create_results_dataframe(results: list, group_value: float | int, config: TableConfig) -> pd.DataFrame:
    ...
    for scenario in scenario_values:
        ...
        for solver in config.solvers:
            result = next((r for r in group_results 
                         if r.solver == solver and config.get_scenario_value(r) == scenario), None)
            if result:
    ...
    columns = pd.MultiIndex.from_tuples(
        [('Scenarios', '')] +
        [(f'Solve Times (seconds)', config.get_solver_name(solver)) for solver in config.solvers] +
        [('Success Rate', config.get_solver_name(solver)) for solver in config.solvers]
    )
